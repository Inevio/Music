...
var loadItem = function( index ){
  ...
  media = new Media( structure.formats['audio/mpeg'].url, function(){
    console.log('play');
  })
...
var parseTime = function( timeSeconds, duration ){

  var time      = parseInt(timeSeconds);
  var hour      = parseInt( time / 3600, 10 );
  var rem       = time % 3600;
  var min       = parseInt( rem / 60, 10 );
  var sec       = parseInt( rem % 60, 10 );
...
.on( 'mousedown touchstart', '.volume-icon', function(){
  ...
  if( win.hasClass('muted') ){
    ...
    media.setVolume( previousVolume );
